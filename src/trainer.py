...
def cal_eps(init_eps, min_eps, max_ep, cur_ep):
    return max(min_eps, init_eps*((max_ep-cur_ep)/max_ep))
...
class Trainer(StateDictMixin):
    def __init__(self, cfg: DictConfig) -> None:
        ...
        #self.agent = Agent_DDPM(instantiate(cfg.agent, num_actions=num_actions)).to(self._device)
        ...
        self._test_collector = make_collector(
            test_env, self.agent.actor_critic, self.policy, self.test_dataset, cfg.collection.test.epsilon, reset_every_collect=True
        )
        ...
        self.opt = CommonTools_denoiser(*map(build_opt, self._model_names))
        self.lr_sched = CommonTools_denoiser(*map(build_lr_sched, self._model_names))
    ...
    def run(self) -> None:
        ...
        while self.epoch < num_epochs:
            ...
            if should_collect_train:
                ...
                to_log += self._train_collector.send((NumToCollect(steps=c.steps_per_epoch), cal_eps(0.5, 0.05, num_epochs, self.epoch)))
    ...
    def collect_initial_dataset(self) -> Tuple[int, Logs]:
        ...
        steps = min_steps
        while True:
            to_log += self._train_collector.send((NumToCollect(steps=steps), 0.5))
    ...
    def collect_test(self, final: bool = False) -> Logs:
        ...
        td.clear()
        to_log = self._test_collector.send((NumToCollect(episodes=episodes),0))
